...
let enhancement1bought = false;

const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));

function updateUI() {
    const clicksEl = document.getElementById("clicks");
    if (clicksEl) clicksEl.innerHTML = Math.round(clicks).toLocaleString() + " Clicks per second: " + Math.round(cps * 200).toLocaleString();
    ...
    if (autoMult && enhancement1bought) {
...
setInterval(() => { //cps but it's in milliseconds so that it feels better on the eyes, and for the gameplay aspect of it
    ...
    cps += clickerUpgrade * (5/1000);
    cps += clickerUpgrade2 * (50/1000);
    cps += clickerUpgrade3 * (250/1000);
    cps += clickerUpgrade4 * (1250/1000);
    cps += cps * totalPercentAddition; //multiplier upgrades increase cps by increasing the multiplier that cps is multiplied by, so this is where the multiplier upgrades take effect
    clicks += cps;
    updateUI();
